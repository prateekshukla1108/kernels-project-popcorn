...
void matrixAdditionKernelByRow(float* C, const float* A, const float* B, int N);

__global__ void matrixAdditionKernelByRow(float* out, float* inp_1, float* inp_2, int n){
    int rowIdx = blockDim.x * blockIdx.x + threadIdx.x;

    if (rowIdx < n ){
        for (int i = 0; i < n; i++){

            out[n * rowIdx + i] = inp_1[n * rowIdx + i] + inp_2[n * rowIdx + i];
        }
    }
}
...
void matrixAddition(float* C, const float* A, const float* B, int N){
    ...
    matrixAdditionKernelByRow<<<gridDim, blockSize>>>(d_C, d_A, d_B, N);
