...
__global__ void matMulTiledUnrolled(const float* A, const float* B, float* C, int n) {
...
int main() {
    ...
    matMulTiledUnrolled << <grid, block >> > (d_A, d_B, d_C, N);
    ...
    printf("Tiled and unrolled multiplication %s\n", correct ? "successful!" : "FAILED");
