...
__global__ void conv1DGpu(const float *x, const float *w, float *y, int N, int K) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;

    // Allocate shared memory for 'w' (weights) and a portion of 'x' (input)
    extern __shared__ float shared_mem[];

    // Shared memory for the filter weights 'w'
    float *shared_w = shared_mem;

    // Shared memory for the input 'x' (only a portion for each block)
    float *shared_x = shared_w + K; // Skip over the weights space

    // Load the weights into shared memory
    if (threadIdx.x < K) {
        shared_w[threadIdx.x] = w[threadIdx.x];
    }
    ...
    // Ensure that all threads have loaded their data into shared memory
    __syncthreads(); 

    if (index < N - K + 1) {
        float result = 0.0f;
        // Perform the convolution using the shared memory data
        for (int i = 0; i < K; ++i) {
            result += shared_x[threadIdx.x + i] * shared_w[i];
        }
        y[index] = result;
    }
}
