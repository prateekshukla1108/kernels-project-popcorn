...
std::vector<PerformanceMetrics> benchmarkMatMul() {
    ...
    PerformanceMetrics naive_metrics = runKernel(MatrixMulNaive, "Naive Implementation", 
                                                d_A, d_B, d_C, h_C_cpu);  // Compare against CPU
    all_metrics.push_back(naive_metrics);
    cudaDeviceSynchronize();
    
    // Run coalesced implementation
    PerformanceMetrics coalesced_metrics = runKernel(MatrixMulTiled, "Tiled Implementation", 
                                                    d_A, d_B, d_C, h_C_cpu);  // Compare against CPU
...
int main() {
    ...
    float baseline_time = metrics[0].kernel_time;
    for(size_t i = 0; i < metrics.size(); i++) {
        const char* name = i == 0 ? "Naive (Baseline)" : "Tiling";
        float speedup = baseline_time / metrics[i].kernel_time;
        printf("%-16s %9.2f %9.2f %8.2fx\n", 
               name, 
               metrics[i].kernel_time,
               metrics[i].gflops,
               speedup);
    }
    printf("============================================\n");
